...
def train(train_file_path, page_num):
    ...
    #make label candidates
    label_list = []

    for item in data:
        label_list.append(str(item['label']))
    ...
    #라벨이 정수형이면 오름차순으로 정렬 (실험 결과 정리 편하기 위함)
    INT_FLAG = True
    #FLAG 변환
    for label in label_list:
        if not label.isdecimal(): #문자열이 int 형으로 변환이 불가능
            INT_FLAG = False
            break
        elif label.isdecimal(): #문자열이 int 형으로 변환이 가능
            pass
    #모든 요소가 int형으로 변환이 가능하면 label_list 정렬
    if INT_FLAG: 
        sorted_label_list = sorted([int(label) for label in label_list])
        label_list = [str(label) for label in sorted_label_list]
    ...
    train_end(args, model, label_list)
...
def train_end(args, model, label_list):
